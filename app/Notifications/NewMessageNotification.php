...
class NewMessageNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
      ...
      return (new MailMessage)
                  ->subject(config('app.name') . ' - Issue ' . $this->issue->reff_no . ' [New Message]')
                  ->line('Issue       : ' . $this->issue->category->name)
                  ->line('Status      : ' . $this->issue->mstatus->status)
                  ->line('Assigned to : ' . $res_name)
                  ->line('Priority    : ' . $this->issue->prioritydesc())
                  ->line('Description : ' . $this->issue->description)
                  ->action('Click here to view message', route('view.message', ['id' => encrypt($this->issue->id), 'mtype' => 1]))
