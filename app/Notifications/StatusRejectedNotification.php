...
class StatusRejectedNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        if ($this->issue->rejected_id == null) {
            $rej_name = '';
        } else {
            $rej_name = $this->issue->reject->name;
        }
        return (new MailMessage)
                    ->subject(config('app.name') . ' - Issue ' . $this->issue->reff_no . ' [Status: Rejected]')
                    ...
                    ->line('Rejected by : ' . $rej_name)
                    ->line('Reason      : ' . $this->issue->reason_for_rejection)
