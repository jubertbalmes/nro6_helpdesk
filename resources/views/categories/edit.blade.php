  ...
  <div class="content rempm">
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-md-12">          
          <form method="post" action="{{ route('category.update', $category) }}" autocomplete="off" class="form-horizontal frm--prevent-multiple-submit">
            @csrf
            @method('put')
              <div class="card mb-8">
                <div class="card-header card-header-rose card-neda">
                  <div class="card-body">
                    <div class="row">
                      ...
                      <h4 class="card-titleX">{{ __('Edit Category') }}</h4>
                    </div>
                  </div>
                </div>
              <div class="card-body ">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                      <textarea cols="30" rows="2" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description" id="input-description" type="text" placeholder="{{ __('Description') }}" required="true" aria-required="true">{{ old('description', $category->description) }}</textarea>
                    ...
                    <div class="form-group{{ $errors->has('main_id') ? ' has-danger' : '' }}">
                      <select class="selectpicker col-sm-12 pl-0 pr-0" name="main_id" data-live-search="true" data-style="select-with-transition" title=" " data-size="6" required="true" aria-required="true">
                        @foreach ($maincategories as $maincategory)
                          <option value="{{ $maincategory->id }}" @if (old('main_id', $category->main_id) == $maincategory->id) selected @endif>{{ $maincategory->name }}</option>
                        @endforeach
                      </select>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('man_day') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('man_day') ? ' is-invalid' : '' }}" name="man_day" id="input-man_day" type="number" placeholder="{{ __('Man-day') }}" value="{{ old('man_day', $category->man_day) }}" required="true" aria-required="true"/>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('ts') ? ' has-danger' : '' }}">
                      <textarea cols="30" rows="10" class="form-control{{ $errors->has('ts') ? ' is-invalid' : '' }}" name="ts" id="input-ts" placeholder="{{ __('Troubleshooting Steps') }}">{{ old('ts', $category->ts) }}</textarea>
